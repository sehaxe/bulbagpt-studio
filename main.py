...
PRESETS = {
    "Micro (3M) - Mac Test": { 
        "h": 64,     # Очень узкая (Embeddings занимают 2MB)
        "i": 256,    # Маленький промежуточный слой
        "l": 4,      # Всего 4 слоя
        "hd": 4,     # 4 головы
        "kv": 2,     # GQA
        "ctx": 128,  # Короткий контекст (быстро)
        "bs": 1,     # Batch Size 1 (Минимум)
        "acc": 16,   # Накапливаем градиент
        "steps": 2000, 
        "lr": 1e-3   # Высокий LR для быстрого обучения малыша
    },
...
def process_data(upload_files, local_path, progress=gr.Progress()):
    if not HAS_RUST: 
        # ЗАМЕНЕНО return -> yield
        yield "❌ Rust Missing", None, 0, pd.DataFrame(), "Error"
        return
    gc.collect()
    ...
    if not files: 
        # ЗАМЕНЕНО return -> yield
        yield f"⚠️ No files in {search_path}", TRAIN_BIN_PATH, 0, pd.DataFrame(), "Error"
        return
    ...
    if not os.path.exists(os.path.join(tok_dir, "tokenizer.model")):
        ...
        try:
            ...
            # ЗАМЕНЕНО return -> yield
            yield f"❌ SPM Train Error: {e}", None, 0, pd.DataFrame(), "Error"
            return
    ...
    # 🔥 ГЛАВНОЕ ИСПРАВЛЕНИЕ ЗДЕСЬ: yield вместо return
